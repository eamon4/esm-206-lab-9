...
homes_sub <- homes %>% 
  filter(City == "Arroyo Grande" | City == "Atascadero" | City == "San Luis Obispo" | City == "Santa Maria-Orcutt")
...
# Note price per sq ft is calculated from the dependent vaiable price. This can lead to biased overfitting of model results 

# Another problem is co-linearity. Bedrooms, bathrooms and sq ft are all indicators of size. Therefore we should just pick one
...
# Coefficent for San Luis Obispo means that we expect a house in San Luis Obispo to sell for $34525.40 more than Arryo Grande holding all else constant  
```
...
# First check class of status column in the console  - note its a character that the lm function is treating as a factor
...
# Overall, heteroscedasticity is ok and residulas look normally distributed. Further, mathmatically and logically the model seems to make sense
```
...
sat_aic <- AIC(homes_lm1) # 10699.24
final_aic <- AIC(homes_lm3) # 11147.99

# Answer: which would you pick? 

# Despite the higher AIC I think the final verson (homes_lm3) because the first itteration has variables that are based on the dependent variable and has issues of co-linearity, making its use inappropriate
...
df_new <- data.frame(City = rep(c("San Luis Obispo",
                     ...
                     SqFt = rep(seq(from = 500,
                                    to = 3500, 
                                    length = 20), # Number of observations
                                times = 12), 
